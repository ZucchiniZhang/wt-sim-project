/**
 * EmptyStateCard - Prompt to generate tactical guide when no cache exists
...
export function EmptyStateCard({ onGenerate, generating, aiEnabled }: EmptyStateCardProps) {
    ...
    return (
      ...
      >
        <div className="text-center">
          <div className="text-aviation-amber text-4xl mb-4">⚙️</div>
          ...
          <p className="text-xs text-aviation-text-muted">
            See <code className="font-mono bg-aviation-slate/50 px-1 py-0.5 rounded">.env.example</code> for full configuration options
          </p>
        </div>
      </motion.div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        {/* Icon */}
        <div className="text-aviation-amber text-5xl mb-4">📋</div>
        ...
        {/* Description */}
        <p className="text-sm text-aviation-text-muted mb-6">
          Generate AI-powered tactical analysis for this aircraft
        </p>
        ...
        <button
          ...
          onClick={onGenerate}
          disabled={generating}
          aria-label={generating ? 'Generating tactical guide, please wait' : 'Generate AI tactical guide for this aircraft'}
