/**
 * FlightAcademyErrorBoundary - Local error boundary for Flight Academy tab
...
interface FlightAcademyErrorBoundaryProps {
  children: ReactNode;
  onRetry?: () => void;
}

export class FlightAcademyErrorBoundary extends Component<
  FlightAcademyErrorBoundaryProps,
  ErrorBoundaryState
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('Flight Academy rendering error:', error, info);
  }
      ...
      return (
        ...
        >
          ...
          <h3 className="text-lg font-header font-bold text-aviation-text mb-2">
            Flight Academy Error
          </h3>
          <p className="text-sm text-aviation-text-muted mb-4">
            {this.state.error?.message || 'A rendering error occurred in the Flight Academy tab.'}
          </p>
          <button
            type="button"
